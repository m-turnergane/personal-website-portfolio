...
export async function generateStaticParams() {
  const slugs = getAllSlugs("trading");
...
export async function generateMetadata({
  ...
  const post = getPostBySlug("trading", slug);

  if (!post) {
    return {
      title: "Trading Lab Not Found",
    };
  }
    ...
    <div className="max-w-4xl mx-auto py-12">
      ...
      <header className="mb-12">
        <div className="flex items-start justify-between gap-4 mb-4">
          <h1 className="text-4xl md:text-5xl font-bold flex-1 min-w-0 break-words text-zinc-100">
            {frontmatter.title}
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-400 mb-6">
          ...
          {frontmatter.status !== "published" && (
            <span className="px-3 py-1 bg-purple-500/20 text-purple-300 border border-purple-500/30 rounded-full text-xs">
              {frontmatter.status}
            </span>
          )}
          {frontmatter.tags.map((tag) => (
            <span
              key={tag}
              className="px-3 py-1 bg-white/[0.03] text-zinc-300 rounded-full text-xs border border-white/10"
            >
              {tag}
            </span>
          ))}
