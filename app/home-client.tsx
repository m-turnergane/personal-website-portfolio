...
import { ConformanceMotif } from "./components/products/conformance-motif";
    ...
    >
      <div className="relative overflow-hidden rounded-xl border border-white/10 bg-white/[0.03]">
        ...
        <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2 px-6 pt-6">
          ...
          <Link
            href="/products"
            className="group inline-flex items-center gap-1 rounded text-sm text-zinc-400 transition-colors hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white/30"
          >
            All products
          ...
          {products.map((product) => (
            <li key={product.slug}>
              <Link
                ...
                className="group grid gap-6 px-6 py-5 transition-colors hover:bg-white/[0.03] focus:outline-none focus-visible:bg-white/[0.03] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white/30 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"
              >
                <div className="min-w-0">
                  <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-zinc-500">
                    Commercial · {product.version}
                  </p>
                  <p className="mt-2 text-lg font-semibold text-zinc-100 group-hover:text-white">
                    {product.name}
                  </p>
                  ...
                  <ul
                    aria-label="Stack"
                    className="mt-3 flex flex-wrap gap-x-3 gap-y-1 font-mono text-[11px] text-zinc-500"
                  >
                ...
                <ConformanceMotif className="hidden md:block opacity-70 transition-opacity group-hover:opacity-100" />
              </Link>
            </li>
          ))}
        </ul>
...
function CategoryGrid({ products }: { products: FeaturedProduct[] }) {
  const categories = [
    {
      title: "Trading Lab",
      description:
